...
Event SearchWindow::onDelete() {
    return Event{ windows::text_editor::events::findReset, windows::text_editor::name, {} };
}
...
Event SearchWindow::processChar(TCPClient& client, const KeyPack& key, const std::string& clipboardData) {
    if (key.keyCode >= 32 && key.keyCode <= 127) {
        ...
        return Event{ windows::text_editor::events::find, windows::text_editor::name, { doc.getText() } };
    }
    switch (key.keyCode) {
    case TABULAR:
        doc.write(0, "    ");
    ...
    case BACKSPACE:
        ...
        return Event{ windows::text_editor::events::find, windows::text_editor::name, { doc.getText() } };
    case ENTER:
        return Event{ windows::text_editor::events::findNext, windows::text_editor::name, {} };
    case ARROW_LEFT:
